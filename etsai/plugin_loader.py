            ...
            try:
                ...
                if hasattr(module, 'register'):
                    ...
                    if isinstance(registered_commands, dict):
                        ...
                        print(f"Warning: The plugin '{item}' did not return a dictionary in 'register()'.")
                else:
                    print(f"Warning: The plugin '{item}' does not have a 'register()' function.")
...
def load_plugins(plugins_dir: str = None) -> Dict[str, Dict[str, Callable]]:
    ...
    if not os.path.exists(plugins_dir) or not os.path.isdir(plugins_dir):
        raise ValueError(f"The specified directory '{plugins_dir}' does not exist or is not valid.")
